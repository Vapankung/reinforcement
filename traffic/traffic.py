...
class TrafficEnvironment:
    def __init__(self, max_steps=1000, variable_patterns=False):
        ...
        self.num_lanes = 2  # Number of lanes per direction
        self.cars = {
            'north': [deque() for _ in range(self.num_lanes)],
            'south': [deque() for _ in range(self.num_lanes)],
            'east': [deque() for _ in range(self.num_lanes)],
            'west': [deque() for _ in range(self.num_lanes)]
        }
        ...
        # Initialize Pygame
        pygame.init()
        self.width = 800  # Increased width for more lanes
        self.height = 800  # Increased height for more lanes
        ...
        self.lane_width = 20
        self.lane_spacing = 5
        self.road_width = self.num_lanes * (self.lane_width + self.lane_spacing)

        # Car settings
        self.car_size = 18  # Slightly smaller to fit lanes
    ...
    def step(self, action, episode):
        ...
        for direction in self.cars.keys():
            ...
            if np.random.rand() < arrival_rate:
                ...
                queue = self.cars[direction][lane]
                ...
                if len(queue) == 0:
                    queue.append({'position': initial_position, 'arrival_time': self.time_step, 'speed': 0})
                else:
                    last_car = queue[-1]
    ...
    def _detect_collisions(self):
        collisions = 0
        for direction, lanes in self.cars.items():
            for queue in lanes:
    ...
    def _get_state(self):
        ...
        state = []
        for lanes in self.cars.values():
            lane_counts = sum(len(queue) for queue in lanes)
            state.append(lane_counts)
        state.extend([self.light_phase, self.current_phase_duration])
        return np.array(state, dtype=np.float32)

    def _update_car_positions(self):
        for direction in self.cars.keys():
            for lane_index, queue in enumerate(self.cars[direction]):
                ...
                for idx, car in enumerate(queue):
                    ...
                    if direction == 'north' and car['position'] < -self.car_size:
                        cars_to_remove.append(car)
                    elif direction == 'south' and car['position'] > self.height + self.car_size:
                        cars_to_remove.append(car)
                    elif direction == 'east' and car['position'] < -self.car_size:
                        cars_to_remove.append(car)
                    elif direction == 'west' and car['position'] > self.width + self.car_size:
                        cars_to_remove.append(car)
    ...
    def render(self, episode):
        ...
        lane_offset = self.lane_width + self.lane_spacing
        for direction, lanes in self.cars.items():
            for lane_index, queue in enumerate(lanes):
                for car in queue:
                    ...
                    if direction == 'north':
                        x = self.width // 2 - (self.road_width // 2) + lane_index * lane_offset + self.lane_spacing
                        y = car['position']
                    elif direction == 'south':
                        x = self.width // 2 + self.lane_spacing + lane_index * lane_offset
                        y = car['position']
                    elif direction == 'east':
                        x = car['position']
                        y = self.height // 2 + self.lane_spacing + lane_index * lane_offset
                    elif direction == 'west':
                        x = car['position']
                        y = self.height // 2 - (self.road_width // 2) + lane_index * lane_offset + self.lane_spacing
                    pygame.draw.rect(self.screen, car_color, (x, y, self.car_size, self.car_size))

        # Draw countdown timer and episode
        timer_text = self.font.render(f"Phase Time Left: {self.max_phase_duration - self.current_phase_duration}", True,
                                      (0, 0, 0))
...
def train_agent(traffic_agent, car_agents, env, episodes, early_stopping_threshold=0.01):
    ...
    for episode in range(episodes):
        ...
        # Print reward and episode in console
